...
try:
    ...
    from diff_gaussian_rasterization import GaussianRasterizationSettings, GaussianRasterizer
from gaussian_renderer._common import infer_gaussian_backend, resolve_gaussian_inputs
...
def render_contrastive_feature(
    ...
    Background tensor (bg_color) must be on GPU!
    """
    backend = infer_gaussian_backend(pc)
    if backend == "2dgs":
        from gaussian_renderer._contrastive_backend_2dgs import render_contrastive_feature_2dgs

        return render_contrastive_feature_2dgs(
            viewpoint_camera,
            pc,
            pipe,
            bg_color,
    ...
    if backend == "3dgs":
        from gaussian_renderer._contrastive_backend_3dgs import render_contrastive_feature_3dgs

        return render_contrastive_feature_3dgs(
            viewpoint_camera,
            pc,
            pipe,
            bg_color,
    ...
    raise ValueError(f"Unsupported gaussian backend '{backend}' for contrastive rendering.")
